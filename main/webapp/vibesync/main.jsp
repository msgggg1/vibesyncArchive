			...
			<nav class="notion-sidebar-container" id="sidebar">
				<div class="notion-sidebar">
					<div class="menu_content">
						...
						<a href="./main.html" class="home icon_wrap"> <img
			...
			<div id="content_wrapper">
				<section id="content">
					...
					<div class="category_btn_group">
						<button
							style="background-image: url(./sources/button_bg/movie.jpg); background-size: cover;">
							<p>Movie</p>
						</button>
						<button
							style="background-image: url(./sources/button_bg/drama.jpg); background-size: cover;">
							<p>Drama</p>
						</button>
						<button
							style="background-image: url(./sources/button_bg/music.jpg); background-size: cover;">
							<p>Music</p>
						</button>
						<button
							style="background-image: url(./sources/button_bg/anime.jpg); background-size: cover;">
							<p>Animation</p>
						</button>
					</div>
					...
					<div class="grid_wrapper">
						<div class="grid_item" id="recent_posts_container">
							<c:forEach items="${recentPostsList}" var="post"
								varStatus="status">
								<div class="list-entry" data-id="${post.note_idx}">
									...
									<a href="<c:url value='/vibesync/noteView.do'><c:param name='noteIdx' value='${post.note_idx}'/></c:url>"class="entry-link"> 
									<img class="entry-image"
										src="https://placehold.co/300x200.png?text=${post.title}"
										alt="${post.title}"> <span class="entry-number">${status.count}.</span><span
						...
						<div class="grid_item" id="popular_posts_container">
							<c:forEach items="${popularPostsList}" var="post"
								varStatus="status">
								<div class="list-entry" data-id="${post.note_idx}">
									<a href="<c:url value='/vibesync/noteView.do'><c:param name='noteIdx' value='${post.note_idx}'/></c:url>"class="entry-link"> 
									<img class="entry-image"src="https://placehold.co/300x200.png?text=${post.title}"alt="${post.title}"> <span class="entry-number">${status.count}.</span><span class="entry-title">${post.title}</span>
									</a>
								</div>
							</c:forEach>
						</div>
						<div class="grid_item" id="popular_users_container">
							<c:forEach items="${popularUsersList}" var="user"
								varStatus="status">
								<div class="list-entry" data-id="${user.ac_idx}">
									<%-- <a href="<c:url value='/vibesync/userProfile.do'><c:param name='acIdx' value='${user.ac_idx}'/></c:url>" class="entry-link"> --%>
									<img class="entry-image"
										src="https://placehold.co/100x100.png?text=${user.nickname}"
										alt="${user.nickname}"> <span class="entry-number">${status.count}.</span>
